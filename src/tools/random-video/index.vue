...
class Box {
    ...
    draw() {
        ctx.value.fillStyle = this.value ? "#000000" : "#ffffff";
        ctx.value.fillRect(this.x, this.y, this.boxSize, this.boxSize);
    }
...
function initCanvas() {
    boxMap.clear();
    const dpr = window.devicePixelRatio || 1;
    const cols = Math.floor(canvasRef.value.width / dpr / dataConfig.size);
    const rows = Math.floor(canvasRef.value.height / dpr / dataConfig.size);

    ctx.value.clearRect(0, 0, canvasRef.value.width / dpr, canvasRef.value.height / dpr);
    ...
    async processVideo(videoElement) {
        ...
        try {
            ...
            while (this.isProcessing) {
                ...
                // 计算适合画布的尺寸
                const targetWidth = Math.floor(canvasRef.value.width / dpr / scaleFactor);
                const targetHeight = Math.floor(canvasRef.value.height / dpr / scaleFactor);
